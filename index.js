    ...
    window.__pmToggleBidirectional = (name) => {
        ...
        if (idx >= 0) arr.splice(idx, 1);
            ...
            if (arr.length >= MAX_BIDIRECTIONAL) {
                ...
                if (status) { status.textContent = `⚠️ 最多同时勾选 ${MAX_BIDIRECTIONAL} 个`; status.style.color = '#cc241d'; }
                return;
            }
            arr.push(name);
        }
...
${currentPersona}：`;
    ...
    window.__pmShowModelPicker = () => {
        ...
        if (!__pmModelList.length) {
            const status = document.getElementById('pm-api-status');
            if (status) { status.textContent = '⚠️ 请先点"连接拉取模型"获取模型列表'; status.style.color = '#d79921'; }
            return;
        }
...
<div class="pm-modal pm-modal-wide">
  ...
  <div class="pm-modal-add" style="display:flex;flex-direction:column;gap:6px;">
    <div style="display:flex;gap:6px;">
      <button onclick="window.__pmTestApi()" style="flex:1;background:#d79921;color:#282828;border:none;border-radius:10px;padding:9px;font-size:12px;cursor:pointer;font-weight:600;">🔗 连接拉取模型</button>
      <button onclick="window.__pmTestModel()" style="flex:1;background:#5856d6;color:#282828;border:none;border-radius:10px;padding:9px;font-size:12px;cursor:pointer;font-weight:600;">🧪 测试模型</button>
    </div>
    <button onclick="window.__pmSaveConfig()" style="background:#689d6a;color:#282828;border:none;border-radius:10px;padding:10px;font-size:13px;cursor:pointer;font-weight:600;">保存配置</button>
  </div>
</div>`);
    };
    ...
    window.__pmTestApi = async () => {
        ...
        const status = document.getElementById('pm-api-status');
        if (!urlInput) { status.textContent = "❌ 请先填写 API 地址"; status.style.color = "#cc241d"; return; }
        status.textContent = "正在测试连接并拉取模型..."; status.style.color = "#689d6a";
    ...
    window.__pmTestModel = async () => {
        ...
        if (!urlInput || !keyInput || !modelInput) {
            ...
            status.style.color = '#cc241d'; return;
        }
        status.textContent = `正在测试模型「${modelInput}」推理...`;
        status.style.color = '#689d6a';
  ...
  <div class="pm-modal-list">
    ...
    <div class="pm-li">
      ...
      <i onclick="window.__pmDel('${n.replace(/'/g,"\\'")}')">删除</i>
        ...
        : '<div style="text-align:center;color:#928374;padding:20px;font-size:13px;">暂无联系人</div>'
    }
  </div>
...
<div class="pm-main-ui">
  <div class="pm-navbar">
    ...
    <div style="display:flex;gap:2px;justify-content:flex-end;">
      ...
      <button onclick="window.__pmEnd()" class="pm-nav-btn" style="color:#cc241d" title="关闭">✕</button>
    </div>
  </div>
...
#pm-iphone {
    ...
    box-shadow: 0 20px 60px rgba(40,40,40,0.85) !important;
...
.pm-name { font-weight: 700 !important; color: #fbf1c7 !important; font-size: 15px !important; text-align: center; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; padding: 0 4px; }
.pm-nav-btn { background: none !important; border: none !important; font-size: 18px !important; cursor: pointer; color: #689d6a !important; padding: 3px !important; line-height: 1; flex-shrink: 0; }
.pm-confirm-bar { background: #2828288f0; border-bottom: 1px solid #d79921; padding: 7px 12px; align-items: center; gap: 8px; flex-shrink: 0; }
.pm-confirm-tip { flex: 1; font-size: 12px; color: #bdae93; }
.pm-confirm-btn { background: #cc241d !important; color: #282828 !important; border: none; border-radius: 8px; padding: 5px 12px; font-size: 12px; cursor: pointer; font-weight: 600; font-family: inherit; }
.pm-cancel-btn { background: #504945 !important; color: #ebdbb2 !important; border: none; border-radius: 8px; padding: 5px 12px; font-size: 12px; cursor: pointer; font-family: inherit; }
...
.pm-checkbox { width: 20px; height: 20px; cursor: pointer; flex-shrink: 0; margin-bottom: 4px; accent-color: #689d6a; opacity: 0.4; transition: opacity 0.15s; }
...
.pm-right { align-self: flex-end !important; background: #689d6a !important; color: #282828 !important; border-bottom-right-radius: 4px !important; }
.pm-left { align-self: flex-start !important; background: #504945 !important; color: #fbf1c7 !important; border-bottom-left-radius: 4px !important; }
.pm-typing-bubble { display: flex !important; gap: 5px; align-items: center; padding: 11px 15px !important; width: fit-content; }
.pm-typing-bubble span { width: 7px; height: 7px; border-radius: 50%; background: #928374; display: inline-block; animation: pm-bounce 1.2s infinite; }
...
.pm-note { text-align: center; font-size: 11px; color: #928374; padding: 3px 0; }
.pm-transfer-card { background: linear-gradient(135deg, #d79921, #d65d0e); color: #282828; border-radius: 14px; padding: 12px 14px; display: flex; align-items: center; gap: 10px; min-width: 150px; box-shadow: 0 3px 10px rgba(215,153,33,0.35); }
...
.pm-img-card { background: #504945; border: 1px solid #665c54; padding: 12px 14px; border-radius: 14px; color: #d5c4a1; font-size: 13px; text-align: center; }
...
.pm-voice-right { background: #689d6a; color: #282828; border-bottom-right-radius: 4px; flex-direction: row-reverse; }
.pm-voice-left { background: #504945; color: #ebdbb2; border-bottom-left-radius: 4px; }
...
.pm-voice-text { background: #3c3836; border: 1px solid #665c54; color: #ebdbb2; padding: 7px 10px; border-radius: 10px; font-size: 13px; line-height: 1.4; max-width: 220px; word-break: break-word; position: relative; }
.pm-voice-text::before { content: '已转文字'; position: absolute; top: -8px; left: 8px; font-size: 9px; color: #928374; background: #282828; padding: 0 4px; border-radius: 4px; }
...
.pm-input { flex: 1 !important; min-width: 0 !important; background: #504945 !important; color: #fbf1c7 !important; border: none !important; border-radius: 20px !important; padding: 9px 14px !important; outline: none !important; font-size: 14px !important; font-family: inherit !important; box-sizing: border-box !important; }
.pm-input:disabled { opacity: 0.5; }
.pm-up-btn { width: 32px !important; height: 32px !important; background: #689d6a !important; color: #282828 !important; border: none !important; border-radius: 50% !important; cursor: pointer; font-size: 16px !important; font-weight: bold; display: flex !important; align-items: center !important; justify-content: center !important; flex-shrink: 0; }
.pm-up-btn:disabled { background: #665c54 !important; cursor: default; }
...
.pm-modal { background: #282828 !important; border-radius: 20px !important; width: 290px; max-height: 85vh; max-height: 85dvh; display: flex !important; flex-direction: column !important; overflow: hidden; box-shadow: 0 16px 48px rgba(40,40,40,0.9); font-family: -apple-system, BlinkMacSystemFont, 'PingFang SC', sans-serif !important; }
...
.pm-modal-header b { font-size: 16px !important; color: #fbf1c7 !important; }
.pm-modal-close { font-size: 20px; color: #928374; cursor: pointer; line-height: 1; }
.pm-bi-bar { padding: 8px 14px; background: #2828288e8; border-bottom: 1px solid #d79921; font-size: 11px; color: #885d00; display: flex; flex-direction: column; gap: 3px; }
...
.pm-li .pm-bi-check { width: 18px; height: 18px; cursor: pointer; flex-shrink: 0; accent-color: #d79921; }
.pm-li span { flex: 1; font-size: 14px !important; color: #689d6a !important; font-weight: 500; cursor: pointer; }
.pm-li i { font-style: normal; font-size: 11px; color: #282828 !important; background: #cc241d !important; padding: 3px 9px; border-radius: 8px; cursor: pointer; font-weight: 600; flex-shrink: 0; }
.pm-modal-add { padding: 12px 14px 16px; border-top: 1px solid #504945; display: flex; gap: 8px; flex-shrink: 0; }
.pm-modal-add input { flex: 1; min-width: 0; border: 1px solid #665c54; border-radius: 10px; padding: 9px 12px; font-size: 13px; outline: none; font-family: inherit; color: #fbf1c7 !important; background: #282828 !important; box-sizing: border-box; }
.pm-modal-add button { background: #689d6a !important; color: #282828 !important; border: none; border-radius: 10px; padding: 9px 14px; font-size: 13px; cursor: pointer; font-weight: 600; white-space: nowrap; font-family: inherit; }
.pm-cfg-label { font-size: 12px; color: #bdae93; margin-bottom: -4px; }
.pm-cfg-input { width: 100%; border: 1px solid #665c54 !important; border-radius: 10px !important; padding: 9px 12px; font-size: 13px !important; outline: none; font-family: inherit; color: #fbf1c7 !important; background: #282828 !important; box-sizing: border-box; }
.pm-cfg-tip { font-size: 11px; color: #928374; text-align: center; padding: 4px 0; }
.pm-mode-switch { display: flex !important; background: #504945; border-radius: 12px; padding: 3px; gap: 3px; }
.pm-mode-opt { flex: 1; text-align: center; padding: 9px 0; font-size: 13px; font-weight: 600; color: #bdae93; cursor: pointer; border-radius: 9px; transition: all 0.2s; user-select: none; }
.pm-mode-opt:hover { color: #d5c4a1; }
.pm-mode-active { background: #282828 !important; color: #689d6a !important; box-shadow: 0 2px 6px rgba(0,0,0,0.3); }
.pm-prof-list { max-height: 130px; overflow-y: auto; border: 1px solid #504945; border-radius: 10px; background: #3c3836; padding: 4px; }
...
.pm-prof-url { font-size: 12px; color: #689d6a !important; font-weight: 600; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
.pm-prof-meta { font-size: 10px; color: #928374; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
.pm-prof-del { font-style: normal; font-size: 12px; color: #cc241d; background: #282828 !important; border: 1px solid #cc241d; width: 22px; height: 22px; border-radius: 50%; display: flex !important; align-items: center !important; justify-content: center !important; cursor: pointer; flex-shrink: 0; font-weight: 600; }
.pm-prof-del:hover { background: #cc241d !important; color: #282828 !important; border-color: #cc241d; }
.pm-prof-empty { text-align: center; color: #928374; font-size: 12px; padding: 14px 0; }

.pm-model-row { display: flex; gap: 6px; }
.pm-model-row .pm-cfg-input { flex: 1; }
#pm-model-arrow { background: #504945; border: 1px solid #665c54; border-radius: 10px; width: 38px; cursor: pointer; font-size: 12px; color: #d5c4a1; flex-shrink: 0; transition: all 0.15s; }
#pm-model-arrow:hover { background: #689d6a; color: #282828; border-color: #689d6a; }
.pm-model-dropdown { position: fixed; z-index: 2147483647; background: #282828 !important; border: 1px solid #665c54 !important; border-radius: 12px !important; box-shadow: 0 8px 24px rgba(40,40,40,0.8); overflow: hidden; display: flex; flex-direction: column; min-width: 200px; padding: 0 !important; margin: 0 !important; color: #fbf1c7 !important; }
.pm-model-search { border: none !important; border-bottom: 1px solid #504945 !important; padding: 9px 12px !important; outline: none; font-size: 13px !important; background: #3c3836 !important; color: #fbf1c7 !important; box-sizing: border-box; width: 100%; font-family: inherit; }
.pm-model-options { overflow-y: auto; max-height: ${MODEL_VISIBLE_ROWS * 34}px; }
.pm-model-opt { padding: 8px 12px; font-size: 13px; color: #ebdbb2; cursor: pointer; border-bottom: 1px solid #3c3836; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; height: 34px; box-sizing: border-box; line-height: 18px; }
.pm-model-opt:hover { background: #f0f7ff; color: #689d6a; }
.pm-model-empty { padding: 14px; text-align: center; font-size: 12px; color: #928374; }
