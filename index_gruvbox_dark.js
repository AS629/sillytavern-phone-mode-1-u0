    ...
    window.__pmToggleBidirectional = (name) => {
        ...
        if (idx >= 0) arr.splice(idx, 1);
            ...
            if (arr.length >= MAX_BIDIRECTIONAL) {
                ...
                if (status) { status.textContent = `⚠️ 最多同时勾选 ${MAX_BIDIRECTIONAL} 个`; status.style.color = '#fb4934'; }
                return;
            }
            arr.push(name);
        }
...
${currentPersona}：`;
    ...
    window.__pmShowModelPicker = () => {
        ...
        if (!__pmModelList.length) {
            const status = document.getElementById('pm-api-status');
            if (status) { status.textContent = '⚠️ 请先点"连接拉取模型"获取模型列表'; status.style.color = '#fabd2f'; }
            return;
        }
...
<div class="pm-modal pm-modal-wide">
  ...
  <div class="pm-modal-scroll">
    ...
    <div style="padding:6px 14px 4px;border-top:1px solid #504945;">
      <div class="pm-cfg-label" style="margin:8px 0 6px;">📚 已保存档案（点击载入）</div>
      <div class="pm-prof-list">${profilesHtml}</div>
    </div>
    <div style="padding:10px 16px;display:flex;flex-direction:column;gap:10px;border-top:1px solid #504945;">
  ...
  <div class="pm-modal-add" style="display:flex;flex-direction:column;gap:6px;">
    <div style="display:flex;gap:6px;">
      <button onclick="window.__pmTestApi()" style="flex:1;background:#fabd2f;color:#282828;border:none;border-radius:10px;padding:9px;font-size:12px;cursor:pointer;font-weight:600;">🔗 连接拉取模型</button>
      <button onclick="window.__pmTestModel()" style="flex:1;background:#d3869b;color:#282828;border:none;border-radius:10px;padding:9px;font-size:12px;cursor:pointer;font-weight:600;">🧪 测试模型</button>
    </div>
    <button onclick="window.__pmSaveConfig()" style="background:#8ec07c;color:#282828;border:none;border-radius:10px;padding:10px;font-size:13px;cursor:pointer;font-weight:600;">保存配置</button>
  </div>
</div>`);
    };
    ...
    window.__pmTestApi = async () => {
        ...
        const status = document.getElementById('pm-api-status');
        if (!urlInput) { status.textContent = "❌ 请先填写 API 地址"; status.style.color = "#fb4934"; return; }
        status.textContent = "正在测试连接并拉取模型..."; status.style.color = "#8ec07c";
    ...
    window.__pmTestModel = async () => {
        ...
        if (!urlInput || !keyInput || !modelInput) {
            ...
            status.style.color = '#fb4934'; return;
        }
        status.textContent = `正在测试模型「${modelInput}」推理...`;
        status.style.color = '#8ec07c';
  ...
  <div class="pm-modal-list">
    ...
    <div class="pm-li">
      ...
      <i onclick="window.__pmDel('${n.replace(/'/g,"\\'")}')">删除</i>
        ...
        : '<div style="text-align:center;color:#a89984;padding:20px;font-size:13px;">暂无联系人</div>'
    }
  </div>
...
<div class="pm-main-ui">
  <div class="pm-navbar">
    ...
    <div style="display:flex;gap:2px;justify-content:flex-end;">
      ...
      <button onclick="window.__pmEnd()" class="pm-nav-btn" style="color:#fb4934" title="关闭">✕</button>
    </div>
  </div>
...
#pm-iphone {
    ...
    background: #282828 !important; border: 10px solid #1d2021 !important;
    border-radius: 45px !important;
    z-index: 2147483647 !important;
    display: flex !important; flex-direction: column !important;
    visibility: visible !important; opacity: 1 !important;
    overflow: hidden !important;
    box-shadow: 0 20px 60px rgba(40,40,40,0.45) !important;
...
.pm-island { width: 100px; height: 28px; background: #1d2021; margin: 8px auto 4px; border-radius: 14px; cursor: move; flex-shrink: 0; touch-action: none; }
.pm-main-ui { flex: 1 !important; display: flex !important; flex-direction: column !important; overflow: hidden; min-height: 0; }
.pm-navbar { display: grid !important; grid-template-columns: 1fr auto 1fr; align-items: center; padding: 6px 10px; border-bottom: 1px solid #504945; flex-shrink: 0; }
.pm-name { font-weight: 700 !important; color: #ebdbb2 !important; font-size: 15px !important; text-align: center; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; padding: 0 4px; }
.pm-nav-btn { background: none !important; border: none !important; font-size: 18px !important; cursor: pointer; color: #8ec07c !important; padding: 3px !important; line-height: 1; flex-shrink: 0; }
.pm-confirm-bar { background: #32302f; border-bottom: 1px solid #504945; padding: 7px 12px; align-items: center; gap: 8px; flex-shrink: 0; }
.pm-confirm-tip { flex: 1; font-size: 12px; color: #a89984; }
.pm-confirm-btn { background: #fb4934 !important; color: #282828 !important; border: none; border-radius: 8px; padding: 5px 12px; font-size: 12px; cursor: pointer; font-weight: 600; font-family: inherit; }
.pm-cancel-btn { background: #504945 !important; color: #d5c4a1 !important; border: none; border-radius: 8px; padding: 5px 12px; font-size: 12px; cursor: pointer; font-family: inherit; }
.pm-msg-list { flex: 1 !important; overflow-y: auto !important; padding: 12px !important; display: flex !important; flex-direction: column !important; gap: 7px; background: #282828 !important; min-height: 0; box-sizing: border-box; }
.pm-select-wrap { display: flex !important; align-items: flex-end; gap: 6px; }
.pm-checkbox { width: 20px; height: 20px; cursor: pointer; flex-shrink: 0; margin-bottom: 4px; accent-color: #8ec07c; opacity: 0.4; transition: opacity 0.15s; }
...
.pm-right { align-self: flex-end !important; background: #8ec07c !important; color: #282828 !important; border-bottom-right-radius: 4px !important; }
.pm-left { align-self: flex-start !important; background: #3c3836 !important; color: #ebdbb2 !important; border-bottom-left-radius: 4px !important; }
.pm-typing-bubble { display: flex !important; gap: 5px; align-items: center; padding: 11px 15px !important; width: fit-content; }
.pm-typing-bubble span { width: 7px; height: 7px; border-radius: 50%; background: #a89984; display: inline-block; animation: pm-bounce 1.2s infinite; }
...
.pm-note { text-align: center; font-size: 11px; color: #a89984; padding: 3px 0; }
.pm-transfer-card { background: linear-gradient(135deg, #fabd2f, #fe8019); color: #282828; border-radius: 14px; padding: 12px 14px; display: flex; align-items: center; gap: 10px; min-width: 150px; box-shadow: 0 3px 10px rgba(250,189,47,0.35); }
...
.pm-img-card { background: #3c3836; border: 1px solid #665c54; padding: 12px 14px; border-radius: 14px; color: #bdae93; font-size: 13px; text-align: center; }
...
.pm-voice-right { background: #8ec07c; color: #282828; border-bottom-right-radius: 4px; flex-direction: row-reverse; }
.pm-voice-left { background: #3c3836; color: #d5c4a1; border-bottom-left-radius: 4px; }
...
.pm-voice-text { background: #32302f; border: 1px solid #665c54; color: #d5c4a1; padding: 7px 10px; border-radius: 10px; font-size: 13px; line-height: 1.4; max-width: 220px; word-break: break-word; position: relative; }
.pm-voice-text::before { content: '已转文字'; position: absolute; top: -8px; left: 8px; font-size: 9px; color: #a89984; background: #282828; padding: 0 4px; border-radius: 4px; }

.pm-input-bar { padding: 8px 12px 30px !important; display: flex !important; gap: 8px; border-top: 1px solid #504945; align-items: center; background: #282828 !important; flex-shrink: 0; box-sizing: border-box; }
.pm-input { flex: 1 !important; min-width: 0 !important; background: #3c3836 !important; color: #ebdbb2 !important; border: none !important; border-radius: 20px !important; padding: 9px 14px !important; outline: none !important; font-size: 14px !important; font-family: inherit !important; box-sizing: border-box !important; }
.pm-input:disabled { opacity: 0.5; }
.pm-up-btn { width: 32px !important; height: 32px !important; background: #8ec07c !important; color: #282828 !important; border: none !important; border-radius: 50% !important; cursor: pointer; font-size: 16px !important; font-weight: bold; display: flex !important; align-items: center !important; justify-content: center !important; flex-shrink: 0; }
.pm-up-btn:disabled { background: #7c6f64 !important; cursor: default; }
...
.pm-modal { background: #282828 !important; border-radius: 20px !important; width: 290px; max-height: 85vh; max-height: 85dvh; display: flex !important; flex-direction: column !important; overflow: hidden; box-shadow: 0 16px 48px rgba(40,40,40,0.28); font-family: -apple-system, BlinkMacSystemFont, 'PingFang SC', sans-serif !important; }
.pm-modal-wide { width: 320px; }
.pm-modal-scroll { flex: 1; overflow-y: auto; min-height: 0; }
.pm-modal-header { display: flex !important; justify-content: space-between !important; align-items: center !important; padding: 16px 18px 12px !important; border-bottom: 1px solid #504945; flex-shrink: 0; }
.pm-modal-header b { font-size: 16px !important; color: #ebdbb2 !important; }
.pm-modal-close { font-size: 20px; color: #a89984; cursor: pointer; line-height: 1; }
.pm-bi-bar { padding: 8px 14px; background: #32302f; border-bottom: 1px solid #504945; font-size: 11px; color: #d79921; display: flex; flex-direction: column; gap: 3px; }
.pm-bi-tip { font-weight: 600; color: #fabd2f; }
...
.pm-li:hover { background: #3c3836; }
.pm-li .pm-bi-check { width: 18px; height: 18px; cursor: pointer; flex-shrink: 0; accent-color: #fabd2f; }
.pm-li span { flex: 1; font-size: 14px !important; color: #8ec07c !important; font-weight: 500; cursor: pointer; }
.pm-li i { font-style: normal; font-size: 11px; color: #282828 !important; background: #fb4934 !important; padding: 3px 9px; border-radius: 8px; cursor: pointer; font-weight: 600; flex-shrink: 0; }
.pm-modal-add { padding: 12px 14px 16px; border-top: 1px solid #504945; display: flex; gap: 8px; flex-shrink: 0; }
.pm-modal-add input { flex: 1; min-width: 0; border: 1px solid #665c54; border-radius: 10px; padding: 9px 12px; font-size: 13px; outline: none; font-family: inherit; color: #ebdbb2 !important; background: #282828 !important; box-sizing: border-box; }
.pm-modal-add button { background: #8ec07c !important; color: #282828 !important; border: none; border-radius: 10px; padding: 9px 14px; font-size: 13px; cursor: pointer; font-weight: 600; white-space: nowrap; font-family: inherit; }
.pm-cfg-label { font-size: 12px; color: #a89984; margin-bottom: -4px; }
.pm-cfg-input { width: 100%; border: 1px solid #665c54 !important; border-radius: 10px !important; padding: 9px 12px; font-size: 13px !important; outline: none; font-family: inherit; color: #ebdbb2 !important; background: #282828 !important; box-sizing: border-box; }
.pm-cfg-tip { font-size: 11px; color: #a89984; text-align: center; padding: 4px 0; }
.pm-mode-switch { display: flex !important; background: #504945; border-radius: 12px; padding: 3px; gap: 3px; }
.pm-mode-opt { flex: 1; text-align: center; padding: 9px 0; font-size: 13px; font-weight: 600; color: #a89984; cursor: pointer; border-radius: 9px; transition: all 0.2s; user-select: none; }
.pm-mode-opt:hover { color: #bdae93; }
.pm-mode-active { background: #282828 !important; color: #8ec07c !important; box-shadow: 0 2px 6px rgba(40,40,40,0.08); }
.pm-prof-list { max-height: 130px; overflow-y: auto; border: 1px solid #504945; border-radius: 10px; background: #32302f; padding: 4px; }
...
.pm-prof-li:hover { background: #282828; }
.pm-prof-info { flex: 1; min-width: 0; cursor: pointer; display: flex; flex-direction: column; gap: 2px; }
.pm-prof-url { font-size: 12px; color: #8ec07c !important; font-weight: 600; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
.pm-prof-meta { font-size: 10px; color: #a89984; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
.pm-prof-del { font-style: normal; font-size: 12px; color: #fb4934; background: #282828 !important; border: 1px solid #cc241d; width: 22px; height: 22px; border-radius: 50%; display: flex !important; align-items: center !important; justify-content: center !important; cursor: pointer; flex-shrink: 0; font-weight: 600; }
.pm-prof-del:hover { background: #fb4934 !important; color: #282828 !important; border-color: #fb4934; }
.pm-prof-empty { text-align: center; color: #a89984; font-size: 12px; padding: 14px 0; }

.pm-model-row { display: flex; gap: 6px; }
.pm-model-row .pm-cfg-input { flex: 1; }
#pm-model-arrow { background: #504945; border: 1px solid #665c54; border-radius: 10px; width: 38px; cursor: pointer; font-size: 12px; color: #bdae93; flex-shrink: 0; transition: all 0.15s; }
#pm-model-arrow:hover { background: #8ec07c; color: #282828; border-color: #8ec07c; }
.pm-model-dropdown { position: fixed; z-index: 2147483647; background: #282828 !important; border: 1px solid #665c54 !important; border-radius: 12px !important; box-shadow: 0 8px 24px rgba(40,40,40,0.18); overflow: hidden; display: flex; flex-direction: column; min-width: 200px; padding: 0 !important; margin: 0 !important; color: #ebdbb2 !important; }
.pm-model-search { border: none !important; border-bottom: 1px solid #504945 !important; padding: 9px 12px !important; outline: none; font-size: 13px !important; background: #32302f !important; color: #ebdbb2 !important; box-sizing: border-box; width: 100%; font-family: inherit; }
.pm-model-options { overflow-y: auto; max-height: ${MODEL_VISIBLE_ROWS * 34}px; }
.pm-model-opt { padding: 8px 12px; font-size: 13px; color: #d5c4a1; cursor: pointer; border-bottom: 1px solid #3c3836; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; height: 34px; box-sizing: border-box; line-height: 18px; }
.pm-model-opt:hover { background: #3c3836; color: #8ec07c; }
.pm-model-empty { padding: 14px; text-align: center; font-size: 12px; color: #a89984; }
